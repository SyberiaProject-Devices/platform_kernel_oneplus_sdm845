...
static int inline synaptics_rmi4_i2c_read_byte(struct i2c_client* client,
		unsigned char addr);

static int inline synaptics_rmi4_i2c_write_byte(struct i2c_client* client,
		unsigned char addr,unsigned char data);

static int inline synaptics_rmi4_i2c_read_word(struct i2c_client* client,
		unsigned char addr);

static int inline synaptics_rmi4_i2c_write_word(struct i2c_client* client,
...
static int tp_baseline_get(struct synaptics_ts_data *ts, bool flag)
{
	...
	ret = synaptics_rmi4_i2c_read_block(ts->client,F54_ANALOG_DATA_BASE+3,2*TX_NUM*RX_NUM,baseline_value);
	for( x = 0; x < TX_NUM; x++ ){
		for( y = 0; y < RX_NUM; y++ ){
			delta_baseline[x][y] =  (int16_t)(((uint16_t)(baseline_value[k])) | ((uint16_t)(baseline_value[k+1] << 8)));
			k = k + 2;
