...
static int find_lowest_rq_pixel(struct task_struct *p, struct cpumask *backup_mask);
...
void select_task_rq_rt_pixel(struct task_struct *p, int prev_cpu, int sd_flag,
			     int wake_flags, int *new_cpu)
{
	...
	*new_cpu = prev_cpu;
	...
	target = find_lowest_rq_pixel(p, &backup_mask);
...
select_task_rq_rt(struct task_struct *p, int cpu, int sd_flag, int flags)
{
	...
	select_task_rq_rt_pixel(p, cpu, flags & 0xF,
		    flags, &target_cpu);
