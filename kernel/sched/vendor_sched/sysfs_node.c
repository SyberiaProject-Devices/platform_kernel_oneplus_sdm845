...
static int proc_dump_task_open(struct inode *inode, struct file *file)
{
	return single_open(file, dump_task_show, NULL);
}

static const struct file_operations proc_dump_task_fops = {
	.open		= proc_dump_task_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
static int create_procfs_node(void)
{
	...
	if (!proc_create("dump_task", 0, vendor_sched, &proc_dump_task_fops)) {
